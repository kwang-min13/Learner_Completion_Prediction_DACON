...
1. `train.csv`, `test.csv`, `sample_submission.csv`를 노트북과 같은 경로에 위치시킵니다.
2. 필요한 라이브러리를 설치합니다:
   ```bash
   pip install catboost scikit-learn pandas numpy seaborn matplotlib
   ```
3. 노트북을 처음부터 순서대로 실행합니다.
4. 최종 결과는 `submission_cb.csv`로 저장됩니다.
...
4. **하이퍼파라미터 튜닝 미실시**
   CatBoost의 기본 파라미터에 `random_seed=42`만 설정하고 학습했습니다. Optuna나 GridSearch를 통한 체계적인 튜닝이 있었다면 성능이 더 개선됐을 것 같습니다.
